...
public class MFPSGeneralDoc : TutorialWizard
{
    ...
    new Steps { Name = "Audio", StepsLenght = 0, DrawFunctionName = nameof(AudioDoc) },
    ...
    new Steps { Name = "Player Animations", StepsLenght = 3, DrawFunctionName = nameof(DrawPlayerAnimationDoc),
    SubStepsNames = new string[]{ "Basic", "Advance", "Weapon Animations" } },
    ...
    new Steps { Name = "Particles & Decals", StepsLenght = 0, DrawFunctionName = nameof(ParticlesDecalsDoc) },
    ...
    void AudioDoc()
    {
        ...
        /*
         DrawText("AUDIO BANKS\n \nAudio Banks is a feature included in MFPS that is not used by default (don't ask me why), but it's really useful if you get to use it, the idea is simple, it's a scriptable object that contains a list of custom class where you can define clip information that can be played in-game from any script without having to create a reference/property in each script, each clip is identified by a custom name.\n \nUsage case of Audio Bank?\nMany, but let me give you a simple example, let's say you create a more interactive Main Menu by adding audio effects to the menu buttons, e.g a sound when click/hover a button, but you want to use different sounds for different types of buttons, to do that you may have to create a script that detects when the user click/hover the button and then play the audio clip, well instead of creating a variable in that script to assign the audio clip per each script, you create an audio bank"); 
          */
    }
    ...
    void DrawKitsSystem()
    {
        DrawText("MFPS have a simple but functional 'Kit System' where players can throw and pick up Ammunition or Medic kits in the map during the game, for default player can throw these kits with the '<b>H</b>' key, the type of kit <i>(ammo or medic)</i> depend of the player class.\n");
        DrawServerImage(7);
        DownArrow();
    ...
    void DrawGameTexts()
    {
        DrawText("If you want to change some text in the game or maybe just change the text grammar, the major part of the text is directly set in the UI Text's components which are located inside the canvas objects in each scene of the game, but also there is some text that is set in runtime by code, to make easy for you to find all these texts we have put all those in just 1 script: <b>bl_GameText.cs</b>\n \nin this script you will find all the text that established by code in runtime, you can modify them as you want from that script, this facilitates the work of for example adding your own localization system.");
    ...
    void ParticlesDecalsDoc()
    {
        ...
        DrawText("<b>NOTE:</b> When you modify a particle prefab, make sure to use the default prefab as a reference since these have attached custom scripts attached aside from the Particle System components which are necessary to the game work correctly.");
        Space(50);
